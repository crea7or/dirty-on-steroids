...
d3.addModule(
{
	...
	run: function()
	{
		if ( d3.page.postComments || d3.page.inboxComments)
		{
			...
			var commentBox = document.querySelector('div.b-comments_add_textarea');
			if ( commentBox )
			{
			...
			{	
			    if ( e.target.nodeName == "A" && e.target.className.indexOf("b-comments_reply_block_close") > -1 )
		    	{
			   		me.addVideoLink( e.target.parentNode );
			   		console.log("found txtarea" + e.target.className );
				}
			    if ( e.target.nodeName == "A" && e.target.className.indexOf("b-close_btn") > -1 )
		    	{
			   		me.addVideoLink( e.target.parentNode );
			   		console.log("found txtarea" + e.target.className );
				}
	...
	addVideoLink: function( commentBody )
	{
		var toolbar = commentBody.parentNode.querySelector('div.b-textarea_editor');
		var me = this;
		var videoLink = document.createElement('a');
		videoLink.href = '#';
		videoLink.textContent = 'Video';
		toolbar.appendChild( document.createTextNode(' '));
		toolbar.appendChild( videoLink );
		...
		var txtArea = toolbar.parentNode.getElementsByTagName('textarea');					
	...
	setCursor: function (txtarea, start, end) {
 	...
 	yarr: function()
 	{
		...
		tagpref = '<a href="' + location.protocol + '//www.youtube.com/watch?v='+me.youtube_id+'&t='+d3.window.seconds+'s">';
